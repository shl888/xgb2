...
class WebSocketPoolManager:
    ...
    def __init__(self, data_callback=None):  # ✅ 修改：参数改为可选
        ...
        if data_callback:
            ...
            logger.info(f"[管理器] WebSocketPoolManager 使用自定义数据回调")
        else:
            # 使用我们修复的默认回调
            self.data_callback = default_data_callback
            logger.info(f"[管理器] WebSocketPoolManager 使用默认数据回调（直接对接共享数据模块）")
        ...
        logger.info("[管理器] WebSocketPoolManager 初始化完成")  # 🎯 统一前缀
    ...
    async def _setup_exchange_pool(self, exchange_name: str):
        ...
        try:
            # 1. 获取合约列表
            logger.info(f"[管理器] [{exchange_name}] 获取合约列表中...")
            ...
            if not symbols:
                logger.error(f"[管理器] [{exchange_name}] 无法获取任何合约，跳过该交易所")
                return
            ...
            if len(symbols) > max_symbols:
                logger.info(f"[管理器] [{exchange_name}] 合约数量 {len(symbols)} > 限制 {max_symbols}，进行裁剪")
                symbols = symbols[:max_symbols]
            
            # 3. 初始化连接池
            logger.info(f"[管理器] [{exchange_name}] 初始化连接池...")
            ...
            logger.info(f"[管理器] ✅ [{exchange_name}] 连接池初始化成功")
    ...
    async def _fetch_exchange_symbols(self, exchange_name: str) -> List[str]:
        ...
        if symbols:
            logger.info(f"[管理器] ✅ [{exchange_name}] 通过API成功获取 {len(symbols)} 个合约")
            return symbols
        
        # 第2步: API失败，使用项目内置的静态列表 (降级)
        logger.warning(f"[管理器] [{exchange_name}] API获取失败，使用内置静态合约列表")
        symbols = self._get_static_symbols(exchange_name)
        logger.info(f"[管理器] ⚠️ [{exchange_name}] 使用静态合约列表，共 {len(symbols)} 个")
        return symbols
    ...
    async def _fetch_symbols_via_api(self, exchange_name: str) -> List[str]:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                logger.info(f"[管理器] [{exchange_name}] 正在加载市场数据... (尝试 {attempt}/{max_retries})")
                ...
                logger.info(f"[管理器] [{exchange_name}] 市场数据加载完成，共 {len(markets)} 个市场")
                ...
                if filtered_symbols:
                    ...
                    logger.info(f"[管理器] [{exchange_name}] 币种分组统计: {group_info}")
                    
                    # 检查是否有重复USDT问题
                    duplicate_usdt_count = sum(1 for s in filtered_symbols if s.upper().endswith('USDTUSDT'))
                    if duplicate_usdt_count > 0:
                        logger.error(f"[管理器] [{exchange_name}] ⚠️ 发现 {duplicate_usdt_count} 个重复USDT的合约!")
                        # 显示有问题的合约
                ...
                if attempt < max_retries:
                    ...
                    logger.warning(f'[管理器] [{exchange_name}] 第{attempt}次尝试失败，{wait_time}秒后重试: {error_detail}')
    ...
    def _filter_and_format_symbols(self, exchange_name: str, markets: dict) -> List[str]:
        """统一的合约筛选与格式化逻辑"""
        all_usdt_symbols = []
        logger.info(f"[管理器] [{exchange_name}] 分析市场中...")
        ...
        for symbol, market in markets.items():
            try:
                ...
                logger.debug(f"[管理器] [{exchange_name}] 处理市场 {symbol} 时跳过: {e}")
                continue
        ...
        if symbols:
            logger.info(f"[管理器] ✅ [{exchange_name}] 发现 {len(symbols)} 个USDT永续合约")
            
            # 打印前10个合约验证格式
            logger.info(f"[管理器] [{exchange_name}] 前10个合约示例: {symbols[:10]}")
        else:
            logger.warning(f"[管理器] [{exchange_name}] 未找到USDT永续合约")
            # 打印一些市场信息帮助调试
            logger.info(f"[管理器] [{exchange_name}] 市场样例 (前5个):")
            count = 0
    ...
    async def shutdown(self):
        ...
        if self._shutting_down:
            logger.info("[管理器] 连接池已在关闭中，跳过重复操作")
            return
        ...
        for exchange_name, pool in self.exchange_pools.items():
            try:
                ...
                logger.error(f"[管理器] [{exchange_name}] 关闭连接池错误: {e}")
