...
class ExchangeWebSocketPool:
    ...
    async def initialize(self, symbols: List[str]):
        ...
        for name, _ in init_tasks:
            logger.info(f"[{self.exchange}] ▶️ 开始初始化 {name}...")
    ...
    async def _enforce_monitor_scheduler(self):
        ...
        if not self.monitor_scheduler_task or self.monitor_scheduler_task.done():
            ...
            logger.info(f"[{self.exchange}] ✅ 监控调度器状态正常")
    ...
    async def _initialize_masters(self):
        ...
        for i, symbol_group in enumerate(self.symbol_groups):
            ...
            try:
                ...
                logger.error(f"[{conn_id}] ❌ 主连接超时30秒，跳过")
            except Exception as e:
    ...
    async def _initialize_warm_standbys(self):
        ...
        for i in range(warm_standbys_count):
            ...
            try:
                ...
                logger.error(f"[{conn_id}] ❌ 温备连接超时30秒，跳过")
            except Exception as e:
            ...
            try:
                ...
                for i, master_conn in enumerate(self.master_connections):
                    if not master_conn.connected:
                        logger.warning(f"[{self.exchange}] 🚨 主连接{i} ({master_conn.connection_id}) 断开")
                ...
                for i, warm_conn in enumerate(self.warm_standby_connections):
                    if not warm_conn.connected:
                        logger.warning(f"[{self.exchange}] 🚨 温备连接{i} ({warm_conn.connection_id}) 断开")
                        await warm_conn.connect()
                        if warm_conn.connected:
                            logger.info(f"[{self.exchange}] ✅ 温备连接{i} 重连成功")
    ...
    async def _select_best_standby_from_pool(self):
        ...
        if not available_standbys:
            logger.warning(f"[{self.exchange}] 温备池无可用连接")
            return None
        ...
        logger.info(f"[{self.exchange}] 🎯 选择最佳温备: {selected_standby.connection_id}")
        return selected_standby
    
    async def _monitor_execute_failover(self, master_index: int, old_master, new_master):
        """监控执行故障转移"""
        logger.info(f"[{self.exchange}] 🚨 故障转移: {old_master.connection_id} -> {new_master.connection_id}")
        
        try:
            # 1. 原主连接降级
            logger.info(f"[{self.exchange}] 步骤1: 原主连接取消订阅")
            ...
            # 2. 温备升级为主
            logger.info(f"[{self.exchange}] 步骤2: 温备升级为主")
            ...
            if not success:
                logger.error(f"[{self.exchange}] 温备切换角色失败")
                return False
            ...
            # 4. 原主连接重连为温备
            logger.info(f"[{self.exchange}] 步骤3: 原主连接重连为温备")
            ...
            if await old_master.connect():
                ...
                logger.info(f"[{self.exchange}] 原主连接已降级为温备")
            
            logger.info(f"[{self.exchange}] ✅ 故障转移完成")
            await self._report_failover_to_data_store(master_index, old_master.connection_id, new_master.connection_id)
            
            return True
            
        except Exception as e:
            logger.error(f"[{self.exchange}] 🔥 故障转移执行失败: {e}")
            return False
    ...
    async def _report_failover_to_data_store(self, master_index: int, old_master_id: str, new_master_id: str):
        ...
        try:
            ...
            logger.info(f"[{self.exchange}] 故障转移记录已保存")
            
        except Exception as e:
            logger.error(f"[{self.exchange}] 保存故障转移记录失败: {e}")
        ...
        while True:
            try:
                ...
                if masters_connected < len(self.master_connections):
                    logger.info(f"[{self.exchange}] {masters_connected}/{len(self.master_connections)} 个主连接活跃")
                
                if warm_connected < len(self.warm_standby_connections):
                    logger.info(f"[{self.exchange}] {warm_connected}/{len(self.warm_standby_connections)} 个温备连接活跃")
                
                await asyncio.sleep(30)
                
            except Exception as e:
                logger.error(f"[{self.exchange}] 健康检查错误: {e}")
                await asyncio.sleep(30)
