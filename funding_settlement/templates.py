...
def get_html_page(manager: Any) -> str:
    ...
    if not contracts_html:
        ...
        <tr>
            <td colspan="4" style="text-align: center; padding: 40px; color: #666;">
                ...
                <div>暂无数据</div>
                <div style="font-size: 14px; margin-top: 10px;">请稍后再试，后台正在获取...</div>
            </td>
        </tr>
        """
    ...
    <body>
        <div class="container">
            ...
            <div class="info-box">
                <strong>💡 说明：</strong> 所有数据来自币安API，每结算周期更新。点击"刷新数据"可立即获取最新数据。
            </div>
        ...
        <script>
            ...
            window.addEventListener('load', function() {{
                if (document.querySelectorAll('#contract-data tr').length <= 1) {{
                    setTimeout(() => {{
                        alert('数据正在后台加载，请稍候...');
                    }}, 1000);
                }}
