...
class DataStore:
    ...
    def set_pipeline_manager(self, pipeline_manager):
        ...
        logger.info("🔗 DataStore: 流水线管理员已连接")
    ...
    def set_brain_callback(self, callback):
        ...
        # ✅ 新增：如果已有流水线管理员，也设置它的回调
        if self.pipeline_manager:
            self.pipeline_manager.set_brain_callback(callback)
    ...
    async def update_market_data(self, exchange: str, symbol: str, data: Dict[str, Any]):
        ...
        # ✅ 修改：如果有流水线管理员，交给管理员处理
        if self.pipeline_manager:
            try:
                await self.pipeline_manager.route_data(data)
                logger.debug(f"[DataStore] 数据已交给流水线处理: {exchange}/{symbol}")
            except Exception as e:
                logger.error(f"流水线处理失败: {e}")
        else:
            logger.debug(f"[DataStore] 流水线管理员未设置，仅存储数据: {exchange}/{symbol}")
    ...
    async def get_pipeline_status(self) -> Dict[str, Any]:
        ...
        if not self.pipeline_manager:
            return {"status": "pipeline_manager_not_set"}
        
        try:
            return await self.pipeline_manager.get_status()
        except Exception as e:
